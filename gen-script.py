script = '''import { mkdir, writeFile } from "node:fs/promises";
...
async function wikiSearch(q) {
  ...
  const r = await fetch("https://en.wikipedia.org/w/api.php?" + params, {headers:{"User-Agent":UA}});
  const d = await r.json(); return d[1]?.[0] ?? null;
}
async function wikiSummary(t) {
  const r = await fetch("https://en.wikipedia.org/api/rest_v1/page/summary/"+encodeURIComponent(t),{headers:{"User-Agent":UA}});
  return r.json();
}
function pickUrl(s, w) {
  const th = s.thumbnail?.source;
  if (th) { const m = th.match(/^(.+\/)([0-9]+)px-([^/]+)$/); if (m) return m[1]+w+"px-"+m[3]; return th; }
...
async function go(query, savePath) {
  try {
    ...
    if (!imgUrl) throw new Error("No image: "+title);
